...
int packets(EthernetConfigs packet, string fileName){
    ...
    cout << "Packet size after preamble = " << length << endl;
    packet.data.push_back("555555D5");
    packet.data.push_back(packet.map["Eth.DestAddress"]);
    packet.data.push_back(packet.map["Eth.SourceAddress"]);

    packet.headerSize = stoi(packet.map["Eth.MaxPacketSize"]) - (packet.map["Eth.DestAddress"].length() + packet.map["Eth.SourceAddress"].length() + 16); //add crc 4 bytes
    int payloadSize = stoi(packet.map["Eth.MaxPacketSize"]) - packet.headerSize;
    packet.data.push_back(iToHex(payloadSize, 2));
